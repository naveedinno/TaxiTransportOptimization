...
class Phase1:
    ...
    def solve(self):
        ...
        for i, trip in enumerate(self.trips):
            ...
            self.G.add_edge("A_start", f"{i}_start", capacity=1)
            self.G.add_edge(f"{i}_end", "A_end", capacity=1)
        ...
        for i, trip1 in enumerate(self.trips):
            for j, trip2 in enumerate(self.trips):
                if (
                    ...
                    self.G.add_edge(f"{i}_end", f"{j}_start", capacity=1)

        self.G.add_edge("A_start", "A_end", weight=-1)
...
if __name__ == "__main__":
    ...
    solver = Phase1(dataset_path, matrixd_path)
    flowCost, flowDict = solver.solve()
    if solver.N < 20:
        solver.plot(flowDict)

    print("The result for Network model (phase 1):")
    print(f"Min number of cars : {solver.N-flowDict['A_start']['A_end']}")